...
class Content(BaseContent):
    ...
    def __replace_by_path(self, data, path):
        ...
        if isinstance(local_data, (list, tuple)):
            if intable_key is not None:
                if len(local_data) <= int(last_key):
                    raise ValueError(f"{'.'.join(_path)}.{last_key} is not a valid index")
                local_data[intable_key] = ANY
            # local_data[int(path_parts[-1])] = ANY
        else:
            if last_key in local_data:
                local_data[last_key] = ANY
            elif local_data.get(intable_key) is not None:
                local_data[to_int(last_key)] = ANY
        return data
